...
# endif
#else
# if (defined(sun) && defined(sparc)) || defined(HAVE_ALLOCA_H)
#  include <alloca.h>
# else
char *alloca ();
# endif
...
#ifdef __linux__
#define	MAX_ALIGNMENT	((mflags & 1) ? (sizeof(double)) : (sizeof (int)))
#endif
...
/* Used to record the GOT entries that need to be relocated. */
struct fixup{
  struct fixup * next;
  symbol * sp;
  struct nlist * p1; /* We use either sp or p1, depending upon which we need */
  struct nlist *p;
  unsigned int first;
  unsigned int second;
  char jump;  /* True if this is a jump instruction that needs to be fixed,
		 false if this is just a pointer */
  char builtin;
};

unsigned int fixup_address = 0;
unsigned int fixup_count = 0;
void write_fixups();
void recompute_nsyms();
struct fixup * fixup_list = NULL;
...
/* got reference prefix */
char *got_ref_prefix = GOT_REF_PREFIX;

/* PLT reference prefix */
char *plt_ref_prefix = PLT_REF_PREFIX;

#define IS_GOT_SYM(sp) \
 (!strncmp((sp)->name, got_ref_prefix, strlen(got_ref_prefix)))

#define IS_PLT_SYM(sp) \
 (!strncmp((sp)->name, plt_ref_prefix, strlen(plt_ref_prefix)))

/* verbose for dll info */
static int dll_verbose = 0;

/* strip GOT and PLT symbols */
static int strip_dll = 1;

#endif

/* Generate warning messages */
void do_warnings (FILE * outfile);
...
#ifdef HOST_TARGET_ENDIANESS_DIFFERS
  entry->strs_size = md_chars_to_number ((unsigned char *) &entry->strs_size,
...
#ifdef HOST_TARGET_ENDIANESS_DIFFERS
  md_chars_to_nlist ((unsigned char *) entry->symbols, entry->symbols,
     ...
     register struct file_entry *entry;
{
  ...
  if (entry->file_type == IS_ARCHIVE)
  ...
  else
    {
      ...
      read_entry_symbols (desc, entry);
      strs = (char *) alloca (entry->strs_size);
      entry->strings = strs;
      read_entry_strings (desc, entry);
...
#ifdef HOST_TARGET_ENDIANESS_DIFFERS
  number_of_symdefs = (md_chars_to_number ((unsigned char *) symdef_data,
					  sizeof (int))
...
#ifdef HOST_TARGET_ENDIANESS_DIFFERS
  md_chars_to_symdefs ((unsigned char *) symdef_base,
		       (struct md_symdef *) symdef_base,
		       number_of_symdefs);
  length_of_strings
    = md_chars_to_number ((unsigned char *) (symdef_base + number_of_symdefs),
			  sizeof (int));
#else
...
void process_subentry (desc, subentry, entry, prev_addr)
     int desc;
     register struct file_entry *subentry;
     struct file_entry **prev_addr, *entry;
{
  ...
  read_entry_symbols (desc, subentry);
  strs = (char *) alloca (subentry->strs_size);
  subentry->strings = strs;
  read_entry_strings (desc, subentry);
...
digest_symbols ()
{
...
#if defined(__linux__)
      if (output_style != OUTPUT_RELOCATABLE) {
	builtin_fixups = getsym_soft("__BUILTIN_FIXUPS__");
	fixup_symbol = getsym_soft("__SHARABLE_CONFLICTS__");
	if(fixup_symbol) set_symbol_count++;
      };
#endif
  ...
  if (output_style != OUTPUT_RELOCATABLE)
    ...
    if (output_style != OUTPUT_RELOCATABLE) {
  /* Now calculate the size of the fixup table.  We look for two
   * different things.  First of all we look for duplicate symbol
   * definitions for symbols that have the __GOT_ or __PLT_ designation.
   * This will come up only when we are linking to multiple shared libraries,
   * or when we are linking a shared library itself.
   *
   * Secondly, we take each symbol in the symbol table, and add the __GOT_
   * and __PLT_ designations, and if we see this it also indicates a fixup
   * that needs to be fixed at run time.
   */

      for (i = 0; i < TABSIZE; i++)
	{
	  register symbol *sp, *sp1;
	  char * cpnt;
	  for (sp = symtab[i]; sp; sp = sp->link)
	    {
	      ...
	      /* This is a special case.  We check for duplicate defs
		 of a GOT symbol where we have no corresponding non-GOT
		 symbol.  This will happen with sharable libraries */
	      if((IS_GOT_SYM(sp) && !getsym_soft(sp->name + 
						strlen(GOT_REF_PREFIX))) || 
		 (IS_PLT_SYM(sp) && !getsym_soft(sp->name +
						strlen(PLT_REF_PREFIX)))) { 

		register struct nlist *def = NULL;
		for (p = sp->refs; p; p = (struct nlist *) p->n_un.n_name)
		...
		for (p = sp->refs; p; p = (struct nlist *) p->n_un.n_name)
		  {
		    if (p != def && p->n_type == (N_ABS | N_EXT))
		      {
			/* Yes, we need a run-time fixup */
			struct fixup * cp;
			cp = (struct fixup *)
			  xmalloc(sizeof(struct fixup));
			cp->next = fixup_list;
			cp->p = p;
			cp->p1 = def;
			cp->sp = NULL;
	      ...
	      if(sp->defined != (N_ABS | N_EXT)){
		if(!IS_GOT_SYM(sp) && !IS_PLT_SYM(sp)){
		  
		  cpnt = (char *) xmalloc(strlen(sp->name)+10);
		  strcpy(cpnt,"__GOT_");
		  strcat(cpnt, sp->name);
		  sp1 = getsym_soft(cpnt);  /* See if a got slot was defined in one
					       of the shared libraries */
		  if(sp1){
		    register struct nlist *next;
		    for (p = sp1->refs; p; p = next)
		      {
			if (p->n_type == (N_ABS | N_EXT))
			  { /* Yes, we need a run-time fixup */
			    struct fixup * cp;
			    cp = (struct fixup *)
			      xmalloc(sizeof(struct fixup));
		  ...
		  /* Now check again for a plt symbol */
		  strcpy(cpnt,"__PLT_");
		  strcat(cpnt, sp->name);
		  sp1 = getsym_soft(cpnt);  /* See if a got slot was defined in one
					       of the shared libraries */
      ...
      fixup_address = data_start + data_size;
      data_size += 2*sizeof(int) +
	(fixup_count) * 2 * sizeof(unsigned int); 
      /* Two longwords */

      if(fixup_symbol)
	fixup_symbol->value = fixup_address;

      if(dll_verbose)
	printf("Fixup count %d\n",fixup_count);
    };  /* Output style != OUTPUT_RELOCATABLE */
#endif
	  ...
	  for (p = sp->refs; p; p = next)
	      ...
	      else if ((type & N_EXT) && type != (N_UNDF | N_EXT))
		{
		  ...
		  if (defs++ && sp->value != p->n_value)
		    {
#if defined(__linux__)
		      /* We do this to prevent multiple symbol def warnings
			 for the __GOT_ and __PLT_ symbols. */
		      if (output_style == OUTPUT_RELOCATABLE ||
			  sp->defined != (N_ABS | N_EXT) || 
			  (!IS_GOT_SYM(sp) && !IS_PLT_SYM(sp)))
#endif
			{
			  sp->multiply_defined = 1;
			  multiple_def_count++;
...
#if defined(__linux__)
	  /* Now we need to add the vector to tell crt0 what to relocate */
	  /* We have to screw with the order to make sure that the dummy
	     version in crt0 is the first one on the list */
	  if(output_style != OUTPUT_RELOCATABLE &&
	     fixup_symbol && fixup_symbol == sp){
	    set_vectors[setv_fill_count] = 
	      set_vectors[setv_fill_count-1];
	    set_vectors[setv_fill_count-1] = fixup_address;
	    setv_fill_count++;
	  };
#endif
     ...
     FILE *outfile;
{
  register struct nlist
    *p,
    *end = entry->symbols + entry->syms_size / sizeof (struct nlist);
  char *strs = (char *) alloca (entry->strs_size);
...
compute_a_out_section_offsets ()
{
  ...
  output_text_offset = N_TXTOFF (outheader);
    ...
    *	H.J.
    */
  if (Tdata_flag_specified || T_flag_specified) {
    long int gap;
...
write_a_out_header ()
{
  ...
  if (need_coff_header)
    {
#ifdef HOST_TARGET_ENDIANESS_DIFFERS
      fatal ("cross linking not supported (coffheader)", (char *) 0);
#endif
  ...
  if (outheader.a_magic == ZMAGIC)
    {
#ifdef HOST_TARGET_ENDIANESS_DIFFERS
      fatal ("cross linking not supported (tek4300 zexec)", (char *) 0);
#endif
  ...
  {
#ifdef HOST_TARGET_ENDIANESS_DIFFERS
    md_hdr_to_chars (&outheader, (unsigned char *) &outheader);
#endif
    mywrite (&outheader, sizeof (struct exec), 1, outdesc);
#ifdef HOST_TARGET_ENDIANESS_DIFFERS
    md_chars_to_hdr ((unsigned char *) &outheader, &outheader);
#endif
  }
...
#ifdef HOST_TARGET_ENDIANESS_DIFFERS
      md_chars_to_relocs ((unsigned char *) reloc,
			  (struct md_reloc_info *) reloc,
			  (entry->text_reloc_size
			   / sizeof (struct md_reloc_info)));
#endif
      entry->textrel = reloc;
    }
...
write_data ()
{
  ...
  if (set_vector_count)
    {
#ifdef HOST_TARGET_ENDIANESS_DIFFERS
      md_vector_to_chars (set_vectors, 2 * set_symbol_count + set_vector_count,
			  (unsigned char *) set_vectors);
#endif
...
#ifdef HOST_TARGET_ENDIANESS_DIFFERS
      {
	unsigned char *loc = (unsigned char *) (data + addr);
	long thing;

	switch (RELOC_TARGET_SIZE(p))
	  {
	  case 0:
	    thing = md_chars_to_number (loc, 1);
	    if (thing & 0x80) /* sign expand */
	      thing |= ~0xff;
	    if (RELOC_MEMORY_SUB_P(p))
	      relocation -= thing;
	    else if (RELOC_MEMORY_ADD_P(p))
	    ...
	    md_number_to_chars (thing, 1, loc);
	    break;

	  case 1:
	    thing = md_chars_to_number (loc, 2);
	    if (thing & 0x8000) /* sign expand */
	      thing |= ~0xffff;
	    if (RELOC_MEMORY_SUB_P(p))
	      relocation -= thing;
	    else if (RELOC_MEMORY_ADD_P(p))
	      relocation += thing;
	    thing = (thing & ~mask) | (relocation & mask);
	    md_number_to_chars (thing, 2, loc);
	    break;

	  case 2:
	    thing = md_chars_to_number (loc, 4);
	    if (thing & 0x80000000) /* sign expand */
	      thing |= ~0xffffffff;
	    if (RELOC_MEMORY_SUB_P (p))
	      relocation -= thing;
	    else if (RELOC_MEMORY_ADD_P (p))
	      relocation += thing;
	    thing = (thing & ~mask) | (relocation & mask);
	    md_number_to_chars (thing, 4, loc);
	    break;

	  default:
	    fatal_with_file ("Unimplemented relocation field length in ",
			     entry);
	  }
      }
#else
...
#endif /* HOST_TARGET_ENDIANESS_DIFFERS */
#endif /* MINIX */
    }
}

...
#ifdef HOST_TARGET_ENDIANESS_DIFFERS
  md_relocs_to_chars ((struct md_reloc_info *) entry->textrel,
		      entry->text_reloc_size / sizeof (struct md_reloc_info),
		      (unsigned char *) entry->textrel);
#endif
...
#ifdef HOST_TARGET_ENDIANESS_DIFFERS
  md_relocs_to_chars ((struct md_reloc_info *) entry->datarel,
		      entry->data_reloc_size / sizeof (struct md_reloc_info),
		      (unsigned char *) entry->datarel);
#endif
...
write_syms ()
{
  ...
  if (strip_symbols == STRIP_ALL) {
    int strtab_size = 4;
#if 0
    fprintf (stderr, "string table offfset: %d\n", output_strs_offset);
#endif
    lseek (outdesc, output_strs_offset, 0);
#ifdef HOST_TARGET_ENDIANESS_DIFFERS
    md_number_to_chars (strtab_size, sizeof (int),
			(unsigned char *) &strtab_size);
#endif
...
#ifdef __linux__
	if (strip_dll && (IS_GOT_SYM(sp) || IS_PLT_SYM(sp))) {
	  continue;
	};
#endif
...
#ifdef HOST_TARGET_ENDIANESS_DIFFERS
  md_number_to_chars (entry->local_syms_offset, sizeof root.ldsymoff,
		      (unsigned char *) &root.ldsymoff);
  md_number_to_chars (entry->text_start_address - entry->orig_text_address,
		      sizeof root.textrel, (unsigned char *) &root.textrel);
  md_number_to_chars (entry->data_start_address - entry->orig_data_address,
		      sizeof root.datarel, (unsigned char *) &root.datarel);
  md_number_to_chars (entry->bss_start_address - entry->orig_bss_address,
		      sizeof root.bssrel, (unsigned char *) &root.bssrel);
  md_number_to_chars (entry->orig_data_address, sizeof root.databeg,
		      (unsigned char *) &root.databeg);
  md_number_to_chars (entry->orig_bss_address, sizeof root.bssbeg,
		      (unsigned char *) &root.bssbeg);
#else
...
perror_name (name)
     char *name;
{
#if !defined(__linux__) || defined(NON_NATIVE)
     ...
     int size;
{
  ...
  register char *result = malloc (size ? size : 1);
#else
...
#if defined(__linux__) && defined(CROSS_LINKER)
int
getpagesize ()
{
  return 4096;
...
#endif
  cp = fixup_list;
  /* Find final addresses of each symbol */
  if(dll_verbose)
    printf("Fixup table address: %x\n",fixup_address);
  while(cp){
    if(!cp->sp)
      cp->sp = (symbol *) cp->p1->n_un.n_name;
    cp->first = cp->sp->value;
    cp->second = cp->p->n_value;
    cp->jump = IS_PLT_SYM(cp->sp);
    /* We need to figure out if this is a jump instruction or a pointer */
    if(!cp->jump && !cp->builtin){
      cpnt = (char *) xmalloc(strlen(cp->sp->name)+10);
      strcpy(cpnt,"__PLT_");
      strcat(cpnt, cp->sp->name);
      sp1 = getsym_soft(cpnt);
      if(sp1 && sp1->value == cp->second) cp->jump = 1;
      free (cpnt);
    };
    cp = cp->next;
  };
/* Check to see if we have multiple relocations of the same address in
   various libraries.  If we do, then fix so that first is always the
   absolute address to be stored.  This way, jump instructions can be repaired
   easily.
*/
  while(cp){
    cp1 = cp->next;
    while(cp1){
      if(cp1->first == cp->second) cp1->first = cp->first;
      cp1 = cp1->next;
    };
    cp = cp->next;
  };
  /* Here we generate a fixup list - this is a list of numbers, we store
     the first number at the address specified by the second number */
  cp = fixup_list;
#ifdef HOST_TARGET_ENDIANESS_DIFFERS
  md_number_to_chars (fixup_count, sizeof (int), (unsigned char *) &xx);
  mywrite ((char *) &xx, sizeof (int), 1, outdesc);
#else
  mywrite((char *) &fixup_count, sizeof(int), 1, outdesc);
#endif
  while(cp){
    if(!cp->builtin){
      if(dll_verbose)
	...
	int new_addr;
	int stored_at;
	stored_at = cp->second+1; /* After jump instruction */
	new_addr = cp->first - (stored_at + 4); /* Relative address */
#ifdef HOST_TARGET_ENDIANESS_DIFFERS
	md_number_to_chars (new_addr, sizeof (int),
			    (unsigned char *) &new_addr);
	md_number_to_chars (stored_at, sizeof (int),
			    (unsigned char *) &stored_at);
#endif
	mywrite((char *) &new_addr, sizeof(int), 1, outdesc);
	mywrite((char *) &stored_at, sizeof(int), 1, outdesc);
      } else {
#ifdef HOST_TARGET_ENDIANESS_DIFFERS
	md_number_to_chars (cp->first, sizeof (int), (unsigned char *) &xx);
	mywrite ((char *) &xx, sizeof (int), 1, outdesc);
	md_number_to_chars (cp->second, sizeof (int), (unsigned char *) &xx);
	mywrite ((char *) &xx, sizeof (int), 1, outdesc);
#else
	mywrite((char *) &cp->first, 2*sizeof(int), 1, outdesc);
#endif
      }
      fixup_count--;
    };
  ...
  if(local_builtins){
    mywrite((char *) zero, 2*sizeof(int), 1, outdesc);
    fixup_count--;
    cp = fixup_list;
    while(cp){
      if(cp->builtin){
/*	cp->first = cp->p1->n_value; */
	if(dll_verbose)
  ...
  if(fixup_count){
    printf("Warning: fixup count mismatch\n");
    while(fixup_count--){
      mywrite((char *) zero, 2*sizeof(int), 1, outdesc);
    };
  };
  ...
  if(builtin_fixups) {
    if(dll_verbose)
      printf("Builtin fixup table at %x\n",builtin_fixups->value);
#ifdef HOST_TARGET_ENDIANESS_DIFFERS
    md_number_to_chars (builtin_fixups->value, sizeof (int),
			(unsigned char *) &xx);
    mywrite ((char *) &xx, sizeof (int), 1, outdesc);
#else
    mywrite((char *) &(builtin_fixups->value), sizeof(int), 1, outdesc);
#endif
  } else
    mywrite((char *) zero, sizeof(int), 1, outdesc);
...
void recompute_nsyms(){
  ...
  if (dll_verbose) printf("Removed %d symbols from symtab\n", count);
}
