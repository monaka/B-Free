...
#define DOS_IPLSIZE(fs)	(fs->iplsize * fs->secsize)
#define DOS_FATENTRY(fs,type)	\
((type == 16) ? \
        (fs->fatsize * fs->secsize / 2) : \
        (((fs->fatsize * fs->secsize) * 2) / 3))
#define DOS_FATSIZE(fs)	(fs->fatsize * fs->secsize)
#define DOS_ROOTDIR(fs)	(fs->rootdir)
#define DOS_ROOTDIROFFSET(fs) \
	(fs->iplsize * fs->secsize)  \
	+ ((fs->fatsize * fs->secsize) * fs->fatcount)
...
extern ER     init_rootdir (struct filesystem *fs);
...
extern W	fname_cmp (B *fname, struct dos_direntry *dir);
